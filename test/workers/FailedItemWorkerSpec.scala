...
import worker.FailedItemWorker

import scala.concurrent.Future

class FailedItemWorkerSpec extends AsyncFreeSpec with AsyncIOSpec
  ...
  private val app = GuiceApplicationBuilder()
    .configure(
      ...
      "workers.failed-item-worker.interval" -> "30 seconds"
    )
    .overrides(
  ...
  private val worker = app.injector.instanceOf[FailedItemWorker]
  ...
  "stream" - {
    ...
    "must start streaming after 1 minute, then call the callback service every 30 seconds" in {
      ...
      when(mockCallbackService.notifyFailedItems()).thenReturn(Future.successful(Done))
      ...
      TestControl.executeEmbed(stream.take(3).compile.toVector).map { results =>
        ...
        verify(mockCallbackService, times(3)).notifyFailedItems()
        succeed
      }
    }
    ...
    "failed calls must not prevent the worker from continuing" in {
      ...
      when(mockCallbackService.notifyFailedItems())
